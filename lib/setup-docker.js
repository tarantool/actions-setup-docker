...
async function run() {
  ...
  if (platform === 'darwin') {
    ...
    await exec.exec('sudo', [
      'usr/local/Caskroom/docker/4.10.1,82475/Docker.app/Contents/MacOS/Docker',
      '--unattended',
    ...
    await exec.exec('open', [
      '-a',
      'usr/local/Caskroom/docker/4.10.1,82475/Docker.app',
      '--args',
      '--unattended',
      '--accept-license'
    ]);
...
i=0
while ! usr/local/Caskroom/docker/4.10.1,82475/Docker.app/Contents/Resources/bin/docker system info &>/dev/null; do
(( i++ == 0 )) && printf %s '-- Waiting for Docker to finish starting up...' || printf '.'
command -v docker || echo 'test docker command loop: not found'
sleep 1
# wait 600s(10min)
if [ $i -gt 600 ];then exit 1;sudo usr/local/Caskroom/docker/4.10.1,82475/Docker.app/Contents/MacOS/com.docker.diagnose check;uname -a;system_profiler SPHardwareDataType;echo "::error::-- Wait docker start $i s too long, exit"; exit 1; fi
done
